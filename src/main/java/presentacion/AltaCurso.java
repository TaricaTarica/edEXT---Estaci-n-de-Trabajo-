...
public class AltaCurso extends JInternalFrame {
	...
	public AltaCurso(IControladorCurso iconCur) {
		...
		lblCantCreditos = new JLabel("Cant. cr√©ditos");
	...
	public boolean comprobarCampos() {
		...
		if (nombre.isEmpty() || descripcion.isEmpty() || duracion.isEmpty() || cantHoras.isEmpty() || cantCreditos.isEmpty() || url.isEmpty() || fecha == null) {
			JOptionPane.showMessageDialog(this, "No puede haber campos vacÌos", "ERROR",
                    JOptionPane.ERROR_MESSAGE);
			return false;
		}
		try {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "La cantidad de crÈditos debe ser un numero", "ERROR",
                    JOptionPane.ERROR_MESSAGE);
            return false;
		}
		return true;
